...
import json
import logging
from chatbox.core import ChatRequest, ChatResponse, SessionStatsResponse
from chatbox.session_manager import SessionManager
from chatbox.game_search import get_game_search
from chatbox.llm.client import LLMStreamingClient
from chatbox.llm.prompts import SYSTEM_PROMPT_GAMING_EXPERT
...
Por favor, responde de manera conversacional y entusiasta. Si encontramos juegos, recomiéndalos explicando por qué son buenas opciones. Si no encontramos juegos, sugiere al usuario que intente con otros términos o géneros."""
    ...
    try:
        ...
        if not games:
            ...
            response_text = f"Encontré estos juegos para tu búsqueda: {game_names}. ¿Te gustaría saber más sobre alguno?"
    session_manager.add_message(session_id=session_id, role="assistant", content=response_text)
    session = session_manager.get_session(session_id) or dict()
    return ChatResponse(
